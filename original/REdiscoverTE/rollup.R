...
option_list <- list(optparse::make_option(c("-m", "--metadata") , dest="metadata_tsv"   , type="character"                                , help="Input metadata .TSV file with two columns, 'quant_sf_path' and 'sample'. See '--help' for details.")
                  , optparse::make_option(c("-t", "--threads")  , dest="num_threads"    , type="integer"     , default=2                  , help="Num processor threads to use. Default is 2.")
                  , optparse::make_option(c("-a", "--assembly") , dest="assembly"       , type="character"   , default='hg38'             , help="Genome assembly version. Must be 'hg38' for now.")
                  , optparse::make_option(c("-d", "--datadir")  , dest="datadir"        , type="character"   , default='rollup_annotation', help="Directory with reference transcriptome annotation data files. This data should have been included with your 'rollup' download.")
                  , optparse::make_option(c("-o", "--outdir")   , dest="outdir"         , type="character"   , default='rollup_output_dir', help="The output directory for Rollup results, which will be created by this program.")
                  , optparse::make_option(c("-p", "--priorcount"),dest="priorcount"     , type="integer"     , default=5                  , help="Prior count used in edgeR::cpm(...) calculation. Default is 5.")
                  , optparse::make_option(c("-v", "--verbose")  , dest="verbose"        , action="store_true", default=FALSE              , help="Prints verbose messages. Useful for debugging.")
                  , optparse::make_option(c("-z", "--nozero")   , dest="remove_zero"    , action="store_true", default=FALSE              , help="Removes any elements (rows) with zero counts. Note: this causes .RDS output files to have differing numbers of rows.")   )
...
mandate <- function(condition, msg) { if (!is.null(condition) && !condition) { warning(msg); stop(msg); } } # similar to stopifnot
...
read_rds_or_table <- function(path, required_cols=c(), ...) {
     if (grepl("[.]rds$", path, ignore.case=T, perl=T)) { # Filename ends in RDS, so try to 'readRDS'
          dat = readRDS(path, ...)
     } else {
...
rollup_and_return <- function(filepaths.vec, outdir, sample_ids.vec, columns, outname, dat.annot) {
     stopifnot(is.character(filepaths.vec)); stopifnot(is.character(sample_ids.vec)); stopifnot(length(sample_ids.vec) == length(filepaths.vec))
...
make_repetitive_element_DGEList <- function(salmon_data, metadata_table, repeat_names_map, dge_fileprefix, outdir) {
     ...
     res.list <- lapply(salmon_data, function(quant) { return(list("repNameNumReads"=quant[["repName"]][, "NumReads", drop=F]
     ...
     repNames.fdat             <- data.frame(repeat_names_map[match(rownames(repName_counts_keep), repeat_names_map$repName), ])  # repNames.fdat appears to be repName, repFamily, repClass for each entry
     ...
     dgelist_repName_counts = edgeR::DGEList(counts=repName_counts_keep, samples=metadata_table, genes=repNames.fdat, remove.zeros=opt$remove_zero)
     saveRDS(dgelist_repName_counts, file=file.path(outdir, paste0(dge_fileprefix, RAW_COUNTS_RDS_SUFFIX)))
     
     dgelist_repName_TPM = DGEList(counts=repName_TPM_keep, samples=metadata_table, genes=repNames.fdat, remove.zeros=opt$remove_zero)
...
make_repName_NormCountDGE_CPM <- function(srcdir, destdir, input_sample_annotated_data_frame, rds_fileprefix) {
     ...
     dge$samples       = as(input_sample_annotated_data_frame, 'data.frame')  # <-- replaces the RE's lib.sizes with the GENE LEVEL counts. This is how we normalize the REs to gene-level totals instead of RE-level totals.
     cpm_matrix        = edgeR::cpm(dge, log=T, prior.count=opt$priorcount)
     cpm_eset          = ExpressionSet(assayData=cpm_matrix, phenoData=input_sample_annotated_data_frame, featureData=AnnotatedDataFrame(dge$genes))
     saveRDS(dge, file=file.path(destdir, paste0(rds_fileprefix, NORM_COUNTS_RDS_SUFFIX))) # <-- note the text 'genenorm' here, indicating that these REs are normalized by *gene* counts
}
...
assert_metadata_tibble_is_ok <- function(meta_tib) {
     mandate(METADATA_QUANTSF_COL %in% colnames(meta_tib), paste0("ERROR: Input metadata file (", opt$metadata_tsv, ") was malformed; it needs a path in a column with this name: ", METADATA_QUANTSF_COL, ". But the only column names we found were: ", paste0(colnames(meta_tib), sep=", ")))
     mandate(METADATA_SAMPLE_COLNAME %in% colnames(meta_tib), paste0("ERROR: Input metadata file (", opt$metadata_tsv, ") was malformed; it needs an ID in a column with this name: ", METADATA_SAMPLE_COLNAME, ". But the only column names we found were: ", paste0(colnames(meta_tib), sep=", ")))
     mandate(!any(is.na(meta_tib[[METADATA_QUANTSF_COL]])), paste0("The quant_sf_path cannot be 'NA', but one of yours is. These must all be specified and be valid files."))
     mandate(all(file.exists(meta_tib[[METADATA_QUANTSF_COL]])), paste0("Every single file that is specified in the quant_sf_path must exist, but the following appear to be missing: ", paste0(meta_tib[[METADATA_QUANTSF_COL]][!file.exists(meta_tib[[METADATA_QUANTSF_COL]])], collapse=", ")))
}

mandate(!is.null(opt$metadata_tsv) && is.character(opt$metadata_tsv), "You must specify a metadata .TSV file with '--metadata=FILENAME'")
mandate(file.exists(opt$metadata_tsv), paste0("Could not read a metadata file at the specified path: ", opt$metadata_tsv))
...
mandate(dir.exists(opt$datadir), paste0("Unable to find specified '--datadir' annotation directory: ", opt$datadir, ". You should specify a valid --datadir=/DIRECTORY/PATH/ "))
...
GLOBAL_NAME_FAMILY_CLASS_MAP <- readr::read_tsv(file.path(opt$datadir, 'repName_repFamily_repClass_map.tsv')); stopifnot(all(colnames(GLOBAL_NAME_FAMILY_CLASS_MAP) == ALL_RE_LEVELS)) # this order is also required
...
for (re_category in c("RE_all", "RE_exon", "RE_intergenic", "RE_intron")) {
     ...
     make_repetitive_element_DGEList(salmon_data=quant_raw_list[[re_category]], metadata_table=metadata, repeat_names_map=GLOBAL_NAME_FAMILY_CLASS_MAP, dge_fileprefix=re_category, outdir=opt$outdir)
}
gene_dgelists.list = make_gene_DGEList(salmon_data=quant_raw_list[["GENE"]], metadata_table=metadata, outdir=opt$outdir)
dgelist_gene_count_rle = edgeR::calcNormFactors(gene_dgelists.list[["DGE_RAW"]], method="RLE") # Note: we calculate the normalization factors based on the GENES, not the REs.
saveRDS(dgelist_gene_count_rle, file=file.path(opt$outdir, paste0("GENE", NORM_COUNTS_RDS_SUFFIX))) # Write the output normalized gene-level data
dgelist_gene_cpm       = edgeR::cpm(dgelist_gene_count_rle, log=T, prior.count=opt$priorcount) # calculate logCPM
geneDGEsamples         = Biobase::AnnotatedDataFrame(dgelist_gene_count_rle$samples) # Used for normalization

for (fname in c("RE_all", "RE_intergenic", "RE_intron", "RE_exon")) {
     make_repName_NormCountDGE_CPM(srcdir=opt$outdir, destdir=opt$outdir, input_sample_annotated_data_frame=geneDGEsamples, rds_fileprefix=fname) # geneDDEsamples is gene-level count data that has been RLE-normalized
}
message("[DONE]")
